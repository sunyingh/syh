...
def parse_args():
    ...
    parser.add_argument("--output_path",
                        type=str,
                        help="directory to save predicted depth maps (will be created if not exists); "
                             "if not set, results are saved next to input images",
                        default=None)
    parser.add_argument("--save_npy",
                        action="store_true",
                        help="if set, also save raw depth/disparity as a .npy file")
...
def test_simple(args):
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            name_dest_im = os.path.join(output_directory, "{}.png".format(output_name))  # 深度图保存格式为 png
