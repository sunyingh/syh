...
import wandb  # 用于在线记录训练过程、配置（超参数），方便后续在网页上做对比和 sweep 调参
...
class Trainer:
    def __init__(self, options):
        ...
        self.models["depth_model"] = endodac.endodac(
            ...
            depth_backbone_ckpt=self.opt.depth_backbone_ckpt)
        ...
        # Wandb 初始化
        # 如果在命令行中传入 --use_wandb True，就会在这里初始化一个 wandb run，
        # 并把当前所有训练配置（超参数）通过 config=vars(self.opt) 记录下来。
        # 这样后续做超参数搜索（sweep）时，可以方便地在 wandb 网页端对比不同 run 的 config 和指标。
        self.use_wandb = getattr(self.opt, "use_wandb", False)
        if self.use_wandb:
            wandb_kwargs = {
                "project": getattr(self.opt, "wandb_project", "EndoDAC_blur"),
                "config": vars(self.opt),
            }
            if getattr(self.opt, "wandb_run_name", None) is not None:
                wandb_kwargs["name"] = self.opt.wandb_run_name
            if getattr(self.opt, "wandb_entity", None):
                wandb_kwargs["entity"] = self.opt.wandb_entity
    ...
    def process_batch_0(self, inputs):
        ...
        for key, ipt in inputs.items():
            # 只对具有 .to 方法的对象（如 Tensor）搬运到指定设备，
            # 避免对 list/str 等类型调用 .to 导致报错
            if hasattr(ipt, "to"):
    ...
    def process_batch_val(self, inputs):
        ...
        """
        for key, ipt in inputs.items():
            # 只对具有 .to 方法的对象搬运到指定设备，避免 list/str 等类型报错
            if hasattr(ipt, "to"):
                inputs[key] = ipt.to(self.device)
            else:
                inputs[key] = ipt
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for tag, value in losses.items():
            self.writers[mode].add_scalar(tag, value, self.step)

        # 如果开启了 wandb，这里会把 loss 和学习率等信息同步到 wandb，
        # 字段名形如 "train/loss"、"val/loss"、"lr/model_0" 等，
        # 方便在 wandb 网页端画出曲线并配合 sweep 功能做自动超参数搜索。
        if getattr(self, "use_wandb", False):
            log_dict = {}
            for k, v in losses.items():
                if hasattr(v, "item"):
                    log_dict[f"{mode}/{k}"] = v.item()
                else:
                    try:
                        log_dict[f"{mode}/{k}"] = float(v)
                    except Exception:
                        pass

            # record learning rates if optimizers exist
            if hasattr(self, "model_optimizer"):
                for i, param_group in enumerate(self.model_optimizer.param_groups):
                    log_dict[f"lr/model_{i}"] = param_group["lr"]
            if hasattr(self, "model_optimizer_0"):
                for i, param_group in enumerate(self.model_optimizer_0.param_groups):
                    log_dict[f"lr/position_{i}"] = param_group["lr"]

            # global step if available
            if hasattr(self, "step"):
                log_dict["global_step"] = self.step
