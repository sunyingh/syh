...
def evaluate(opt):
    ...
    cls_1 = st.t.interval(0.95, len(ates_1)-1, loc=np.mean(ates_1), scale=st.sem(ates_1))
    cls_1 = np.array(cls_1)
    cls_2 = st.t.interval(0.95, len(ates_2)-1, loc=np.mean(ates_2), scale=st.sem(ates_2))
