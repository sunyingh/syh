...
class _RFARefine(nn.Module):
    def __init__(self, channels: int):
        super().__init__()
        self.rfa = ConvMod(channels)
...
class DPTHead(nn.Module):
    def __init__(self, in_channels, features=128, use_bn=False, out_channels=[96, 192, 384, 768], use_clstoken=False):
        ...
        self.rfa_refine = nn.ModuleList([
            _RFARefine(out_channel) for out_channel in out_channels
        ])
