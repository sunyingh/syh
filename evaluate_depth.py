...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        # 根据 eval_split 选择对应的数据集
        # 这里将 "endovis" 和任何基于 endovis 的变体（如 endovis_blur）
        # 统一走 SCAREDRAWDataset 的评估逻辑，只是使用不同的 splits 子目录
        if opt.eval_split.startswith('endovis'):
    ...
    # 对所有以 endovis 开头的 eval_split（如 endovis, endovis_blur, endovis_xxx），
    # 都从对应的 splits 子目录中加载 gt_depths.npz 作为深度真值
    if opt.eval_split.startswith('endovis'):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader)):
            ...
            # 以 endovis 系列（包括 endovis_blur 等）使用预先导出的 gt_depths.npz
            if opt.eval_split.startswith('endovis'):
    ...
    for i in range(len(mean_errors)):
        # 兼容当前 scipy 版本的 t.interval 接口：第一个参数为置信度，第二个为自由度
        cl = st.t.interval(0.95, len(errors) - 1,
                           loc=mean_errors[i],
                           scale=st.sem(errors[:, i]))
        cls.append(cl[0])
        cls.append(cl[1])
    cls = np.array(cls)
